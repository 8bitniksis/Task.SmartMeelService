...
namespace Sms.GrpcClientLibrary
{
    ...
    public class GrpcApiClient : IDisposable
    {
        ...
        public async Task<bool> SendOrderAsync(Order order, CancellationToken cancellationToken = default)
        {
            ...
            var protoOrder = new Protos.Order { Id = order.OrderId };
            foreach (var item in order.Items)
            {
                protoOrder.OrderItems.Add(new Protos.OrderItem { Id = item.Id, Quantity = item.Quantity });
            }

            try
            {
                var response = await _client.SendOrderAsync(protoOrder, cancellationToken: cancellationToken);
